...
public class MySQLConnection {
    ...
    public MySQLConnection() throws MySQLException {
        try {
            Class.forName("com.mysql.cj.jdbc.Driver").newInstance();//reflect mechanism
    ...
    // save resource of both SQL and local memory;
    public void close() {
        if (conn != null) {
            try {
                conn.close();
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        saveItem(item);// insert item into item table

        // insert user id and item id to favorite table
//        String sql = String.format("INSERT INTO favorite_records (user_id, item_id) VALUES (%s, %s)", userId, item.getId());
        String sql = "INSERT IGNORE INTO favorite_records (user_id, item_id) VALUES (?,?)";// more safe, by using SQL checking mechanism
        try {
