...
public class ItemRecommender {
    ...
    private List<Item> recommendByTopGames(ItemType type, List<Game> topGames) throws RecommendationException{
        ...
        for (Game game: topGames){
            ...
            try{
                ...
                throw new RecommendationException("Fail to recommend by top games");
            }
    ...
    private List<Item> recommendByFavoriteHistory(
        ...
        // count all the game ids
        Map<String, Long> favoriteGameIdsByCount =
                favoriteGameIds.parallelStream().collect(Collectors.groupingBy(Function.identity(),Collectors.counting()));

        // sorting
        List<Map.Entry<String, Long>> sortedFavoriteGameIdsByCount = new ArrayList<>(favoriteGameIdsByCount.entrySet());
        sortedFavoriteGameIdsByCount.sort((Map.Entry<String, Long> e1, Map.Entry<String, Long> e2) -> Long.compare(e2.getValue(), e1.getValue()));

        if(sortedFavoriteGameIdsByCount.size() > DEFAULT_GAME_LIMIT){
            sortedFavoriteGameIdsByCount = sortedFavoriteGameIdsByCount.subList(0, DEFAULT_GAME_LIMIT);
        }

        outerloop:
        for (Map.Entry<String, Long> entry: sortedFavoriteGameIdsByCount){
            List<Item> items;
            try{
